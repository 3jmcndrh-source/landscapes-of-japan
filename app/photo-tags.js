...
export const PHOTO_TAGS = {
  ...
  // ── 北海道/札幌 中島公園 (2026-07-25) — アオサギ1 + アカゲラ6、全て目視確認 ──
  // 札幌は night-views.locs に入っているため、無タグだと鳥写真が夜景に混入する
  "dsc06513-991893": ["bird"],   // アオサギ
  "dsc06597-6d887f": ["bird"],   // アカゲラ
  "dsc06625-8d8ea9": ["bird"],
  "dsc06649-1b477c": ["bird"],
  "dsc06690-7bf9e8": ["bird"],
  "dsc06702-1710c2": ["bird"],
  "dsc06726-f68ae3": ["bird"],   // アカゲラ 巣穴

  // ── 神奈川県/横浜 (2026-08-07〜09、26枚 全て目視確認) ──
  // 横浜も night-views.locs 入り。夜景は実際に夜/薄暮の4枚だけに限定する
  "dsc07051-78679e": ["urban"],              // 観覧車と街 昼
  "dsc07054-8754f9": ["urban"],              // 街並み俯瞰 昼
  "dsc07058-7516d0": ["urban"],              // 観覧車 昼
  "dsc07069-8bc01e": ["urban", "coastal"],   // ランドマークタワー逆光 海越し
  "dsc07076-d3287f": ["urban", "street"],    // 赤レンガ倉庫
  "dsc07084-e7251e": ["urban", "night"],     // 薄暮のみなとみらい 灯り点灯
  "dsc07086-2efab4": ["night", "urban"],     // 観覧車 夜景
  "dsc07088-4bd354": ["coastal"],            // 夕日と海 工場シルエット
  "dsc07089-73a4d4": ["night", "urban"],     // 観覧車と街 夜景
  "dsc07093-364df3": ["night", "urban"],     // みなとみらい 夜景
  "dsc07101-70d48f": ["urban"],              // ランドマークタワーと水辺 夕
  "dsc07105-8b2743": ["coastal"],            // 夕日と海 桟橋
  "dsc07109-be175e": ["coastal"],            // 海と桟橋 曇天
  "dsc07110-6918a8": ["urban"],              // ホテル建築 見上げ
  "dsc07112-a46dd6": ["urban"],              // 観覧車と街 昼
  "dsc07118-a2ae52": ["coastal"],            // 夕日と海
  "dsc07119-1057a3": ["coastal"],            // 夕日と海 工場
  "dsc07129-6e8597": ["coastal"],            // 灯台と夕日
  "dsc07145-948ed8": ["coastal"],            // 桟橋と夕日
  "dsc07152-bde6ca": ["coastal"],            // 海沿いの護岸 昼
  "dsc07156-461a98": ["coastal"],            // 海岸線の遊歩道 昼
  "dsc07166-02f7a6": ["urban"],              // ホテル建築 昼
  "dsc07179-d3840d": ["coastal"],            // 客船と海
  "dsc07187-a8ac36": ["urban"],              // 観覧車 見上げ 昼
  "dsc07198-05ad43": ["urban"],              // 観覧車と街 昼
  "dsc07200-804e4b": ["urban"],              // 街並み俯瞰 昼

  // ── 群馬県/草津温泉 (2026-08-08、30枚 全て目視確認) ──
  // 湯畑・西の河原が中心。7463/7464 は帰路のダム湖 (loc はユーザー指定どおり草津温泉)
  "dsc07213-3ac7f8": ["onsen"],                 // 湯畑 朝
  "dsc07233-db590c": ["street"],                // ご当地マンホール
  "dsc07261-9f2642": ["onsen", "night"],        // 湯畑 ライトアップ
  "dsc07280-e4d7b4": ["onsen", "night"],        // 西の河原 ライトアップ
  "dsc07283-ece14d": ["onsen"],                 // 湯畑 夕景
  "dsc07285-e173c6": ["onsen", "night"],        // 湯畑 薄暮
  "dsc07295-d699b7": ["waterfall", "onsen"],    // 湯滝
  "dsc07296-69f0da": ["waterfall", "onsen"],    // 湯滝 スローシャッター
  "dsc07297-f02cfc": ["waterfall", "onsen"],    // 湯滝と街並み
  "dsc07307-fe17fc": ["onsen"],                 // 西の河原 湯池
  "dsc07309-5af6c3": ["street"],                // 温泉街の建物
  "dsc07312-24f17d": ["street"],                // 木造建築
  "dsc07315-210df3": ["street"],                // 温泉街俯瞰 夕
  "dsc07317-b05528": ["street"],                // 温泉街 夕焼け
  "dsc07324-39991b": ["street"],                // 商店
  "dsc07327-f2477e": ["street"],                // 店先の暖簾
  "dsc07337-80289c": ["temple"],                // 地蔵尊
  "dsc07344-ba0cae": ["onsen"],                 // 西の河原 湯池
  "dsc07351-c7e437": ["flower-field"],          // 睡蓮の池
  "dsc07357-0bfe5a": ["landscape"],             // 森の遊歩道
  "dsc07365-40dd78": ["onsen"],                 // 西の河原 湯けむりと岩
  "dsc07367-c10ca0": ["onsen"],                 // 湯けむりと光芒
  "dsc07378-11c479": ["street"],                // 温泉街の路地
  "dsc07381-5a1558": ["animal"],                // 猫
  "dsc07392-594335": ["waterfall", "onsen"],    // 湯滝と街並み 夕
  "dsc07398-d6d7d7": ["onsen"],                 // 湯畑と温泉街
  "dsc07411-870324": ["onsen"],                 // 湯畑
  "dsc07450-a01d07": ["onsen"],                 // 湯樋 接写
  "dsc07463-5c0bb5": ["lake", "mountain"],      // ダム湖と山
  "dsc07464-381182": ["lake", "mountain"],      // ダム湖と山

  // ── 東京都/東京 (2026-08-09、8枚 全て目視確認) ──
  // 東京も night-views.locs 入りだが 8枚すべて日中。無タグだと全部夜景に混入した
  "dsc07467-725d89": ["urban"],              // 東京タワー 見上げ
  "dsc07470-296bdc": ["urban"],              // 東京タワー 緑越し
  "dsc07475-67b5e7": ["temple", "urban"],    // 増上寺と東京タワー
  "dsc07491-4781ef": ["temple", "urban"],    // 増上寺と東京タワー
  "dsc07508-292f93": ["coastal", "urban"],   // お台場 海とレインボーブリッジ
  "dsc07518-191a3e": ["urban"],              // お台場の街
  "dsc07520-6fbfaa": ["urban", "coastal"],   // 自由の女神とレインボーブリッジ
  "dsc07522-274cd5": ["urban"],              // フジテレビ社屋

  // ── 北海道/室蘭 (2026-08-10、7枚 全て目視確認) ── 全て海と夕景
  "dsc07525-ebc24c": ["coastal"],
  "dsc07527-f8e588": ["coastal"],   // 夕日の海
  "dsc07531-e6adcc": ["coastal"],
  "dsc07537-fe736a": ["coastal"],
