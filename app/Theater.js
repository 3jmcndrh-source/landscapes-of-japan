...
export default function Theater({ photos, lang, onClose, labels, startIndex = 0, initialShuffle = false }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={ui("theaterMode", lang)}
    >
      {prev && tick > 0 && (
      ...
      <div className={"th-ui" + (uiVisible ? " visible" : "")}>
        <button className="th-close" onClick={stop(onClose)} aria-label={ui("close", lang)}>×</button>
        <div className="th-controls" onClick={(e) => e.stopPropagation()}>
          <button className="th-btn" onClick={() => advance(-1)} aria-label={ui("previousPhoto", lang)}>
          ...
          <button className="th-btn" onClick={() => advance(1)} aria-label={ui("nextPhoto", lang)}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
          </button>
          <button className={"th-btn th-shuffle" + (shuffleOn ? " active" : "")} onClick={toggleShuffle} aria-label={ui("shuffle", lang)} title={ui("shuffle", lang)}>
