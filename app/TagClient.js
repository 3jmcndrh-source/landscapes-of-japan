...
export default function TagClient({ lang, slug, photos, desc }) {
  ...
  // 関連タグ (上限8)
  const relatedTags = TAG_SLUGS.filter((s) => s !== slug).slice(0, 8);
    ...
    <div style={{ background: "#0a0a0a", color: "#e8e4df", minHeight: "100vh", fontFamily: "'Cormorant Garamond',Georgia,serif" }}>
      <div className="top-bar scrolled">
        <div className="top-langs">
          {Object.entries(TR).map(([c]) => (
            <a key={c} href={`/${c}/tags/${slug}`} className={"top-lang-btn" + (lang === c ? " active" : "")}>
              {TR[c].name}
            </a>
          ))}
        </div>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 24px 80px" }}>
        ...
        <header style={{ marginBottom: 40 }}>
          <div style={{ fontFamily: "var(--font-zen-kaku),sans-serif", fontSize: 11, letterSpacing: ".25em", textTransform: "uppercase", color: "rgba(220,190,100,.7)", marginBottom: 8 }}>{tagsLabel}</div>
          ...
          <div style={{ fontFamily: "var(--font-zen-kaku),sans-serif", fontSize: 14, color: "rgba(232,228,223,.55)", marginTop: 10, letterSpacing: ".05em" }}>
            {photos.length} {lang === "ja" ? "枚" : "photos"}
          </div>
        </header>
        ...
        <section style={{ marginTop: 72 }}>
          <h2 style={{ fontFamily: "var(--font-zen-kaku),sans-serif", fontSize: 14, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(220,190,100,.7)", marginBottom: 20 }}>
            {lang === "ja" ? "関連タグ" : "Related Tags"}
          </h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {relatedTags.map((s) => (
              <a key={s} href={`/${lang}/tags/${s}`} style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(220,190,100,.15)", borderRadius: 999, padding: "8px 16px", color: "#e8e4df", textDecoration: "none", fontFamily: "var(--font-zen-kaku),sans-serif", fontSize: 13 }}>
                #{getTagName(s, lang)}
              </a>
            ))}
          </div>
